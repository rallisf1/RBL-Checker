...
type Datatable= {
    ...
    action?:boolean,
    EditRedirect?:string|number|undefined
}

let {headers,action=false,page_number="5",Rows,onEditClick,onDeleteClick,EditRedirect}:Datatable=$props()
                    ...
                    <tr>
                        ...
                        {#if action}
                            <td class="text-center flex justify-center align-center">
                                {#if onEditClick}   
                                <button class="btn btn-circle btn-text btn-sm" aria-label="Action button" onclick={()=>onEditClick(row[0])}>
                                    <a href={EditRedirect != null ? String(EditRedirect) : undefined} aria-label="Edit"><span class="icon-[tabler--pencil] size-5"></span></a>
                                </button>
                                {:else}
                                ...
                                {/if}
                                {#if onDeleteClick}
                                <button class="btn btn-circle btn-text btn-sm" aria-label="Action button" onclick={()=>onDeleteClick(row[0])}>  
